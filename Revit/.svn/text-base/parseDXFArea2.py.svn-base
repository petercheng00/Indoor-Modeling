...
mm = 1 # if in millimeters
RevitScale = (10.0/1013244.36798) * mm
vertexDistThresh = 0.001 * mm
planeDistThresh = 0.001 * mm
sizeThresh = 0.001 * mm


def within(val1, val2, e=.00001):
    if (val1 > val2):
        return (val1 - val2) < e
    else:
        return (val2 - val1) < e
...
def validVertex(v, plane):
    ...
    if len(plane) >= 1:
        if ((within(prevVertex[0], v[0], vertexDistThresh)) and
            (within(prevVertex[1], v[1], vertexDistThresh)) and
            (within(prevVertex[2], v[2], vertexDistThresh))):
            return False
    if len(plane) >= 2:
